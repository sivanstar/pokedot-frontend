...
export const Dashboard: React.FC = () => {
  ...
  // Check if task is needed on mount
  useEffect(() => {
    const checkTaskStatus = async () => {
      try {
        ...
        if (data.success && data.needsTask) {
  ...
  const handleTaskComplete = async () => {
    try {
      ...
      if (data.success) {
        ...
        toast.success('Task completed! Welcome to your dashboard.', {
          duration: 3000,
          icon: '✅'
        });
  ...
  const handleTaskSkip = () => {
    ...
    toast.info('You can access the platform after completing the task', {
      duration: 5000
    });
  };
